...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const mapLP = { "endowment":"Endowment Fund","sovereign":"Sovereign Wealth Fund","bank":"Bank",
    ...
    const mapGP = { "private equity":"Private Equity","pe":"Private Equity","venture capital":"Venture Capital",
      "vc":"Venture Capital","angel":"Angel Investors","corporate":"Corporate Development Team",
      "cvc":"Corporate Development Team","incubator":"Incubator","sbic":"SBIC","bdc":"Business Development Company",
      "growth":"Growth Equity Firm","accelerator":"Accelerator","fof":"Fund of Funds","angel group":"Angel Group",
      "asset":"Asset Management Firm","angel fund":"Angel Investment Fund" };
    const mapSect = { "energy":"Energy","materials":"Materials","industrials":"Industrials",
    ...
    if (entityType === "LP") {
      const k = Object.keys(mapLP).find(k => lc(subType).includes(k));
      subType = k ? mapLP[k] : "Other";
    } else if (entityType === "GP") {
      const k = Object.keys(mapGP).find(k => lc(subType).includes(k));
      subType = k ? mapGP[k] : "Other";
    } else subType = "Other";

    { const k = Object.keys(mapSect).find(k => lc(sector).includes(k));
      sector = k ? mapSect[k] : "Sector Agnostic"; }

    if (!geo) return json({ error: "geo is required" }, 400);

    /* —— Gemini prompt —— */
    const PROMPT = `
Return ONLY a pure JSON array (no markdown) of 5 firms that fit:
...
`;
    ...
    let text = gJson?.candidates?.[0]?.content?.parts?.[0]?.text || "[]";
    /* strip ``` fences if any */

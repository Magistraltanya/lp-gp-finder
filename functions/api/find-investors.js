...
export async function onRequestPost({ request, env }) {
  try {
    ...
    /* ── [NEW] System Instruction to lock the AI into JSON-only mode ── */
    ...
    /* ── [NEW] "Fill-in-the-Blanks" Prompt Template ── */
...
`;
    ...
    for (let i = 0; i < 3; i++) {
      res = await fetch(url, {
        ...
        body   : JSON.stringify({
          contents: [
            systemInstruction, // System instruction sets the AI's role
            { role: 'user', parts: [{ text: PROMPT_TEMPLATE }] } // User prompt is the template to fill
          ],
